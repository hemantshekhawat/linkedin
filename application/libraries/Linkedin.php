...
define('CUSTOMER_KEY', 'sqpx0htdt6nh');
define('CUSTOMER_SECRET', 'dePDmfiDc7P8K4xP');
...
class Linkedin {
	...
	function get_curl_response($toHeader, $url, $post = true) {
	        ...
	        curl_setopt($ch, CURLOPT_HTTPHEADER, array($toHeader,"x-li-format: json"));
	...
	function get_profile($requestURL) {
		...
		$endpoint = $requestURL.":(".join(',', $this->profileFields).")";
		...
		//$t = $this->parse_xml($this->get_curl_response($toHeader, $endpoint, false));
                
                $output = $this->get_curl_response($toHeader, $endpoint, false);
//			echo "<pre>";
//			print_r($output);
//			echo "</pre>";
	...
	function get_logged_in_users_profile() {
		return $this->get_profile(BASE_API_URL . '/v1/people/~');
	}
